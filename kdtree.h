				 ...
				 class KDTree
{
	...
	private:
		 ...
		 */
		int _euclideanDistance(const Point<Dim> & first, const Point<Dim> & second) const;

		/* Helper function that constructs a sub-tree of a KDTree by taking in
		 * a left and right boundary as well as the current dimension.
		 */
		void _KDTree(int left, int right, int curDim);

		/* Calculates the relative median given left and right boundaries. */
		static int _median(int left, int right);

		/* Partition function that groups a list into two parts, those less
		 * than a certain element (pivotIndex) and those greater than or equal
		 * to the element, by taking in left and right boundaries as well as
		 * the current dimension.
		 */
		int _partition(int left, int right, int pivotIndex, int curDim);

		/* Select function that returns the k-th element in a list given
		 * left and right boundaries as well as the current dimension.
		 */
		Point<Dim> _select(int left, int right, int k, int curDim);

		/* Helper function for findNearestNeighbor that returns the position
		 * of the nearest neighbor to a query point in a KDTree by taking in
		 * left and right boundaries, the current point as well as the
		 * current dimension.
		 */
		int _findNearestNeighbor(const Point<Dim> & query, int curr, int left, int right, int curDim) const;
};
